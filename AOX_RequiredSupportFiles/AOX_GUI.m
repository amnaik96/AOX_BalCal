...
[FileName, PathName] = uigetfile(fspec,'Pick Validation File');
if FileName ~= 0
    [CurrentPath,~,~] = fileparts(mfilename('fullpath'));
...
[FileName, PathName] = uigetfile(fspec,'Pick Approx File');
if FileName ~= 0
    [CurrentPath,~,~] = fileparts(mfilename('fullpath'));
    CurrentPath = [CurrentPath,'\'];
    if strcmp(CurrentPath,PathName)
...
%                 if isempty(p1_split) == 0 && isempty(p2_split) == 0 && isempty(p3_split) == 0
%                     % split data arrays into part 1 and part 2
%                     part1_excv  = excessVec0(1:p2_split-1,:); % part 1 (applied loads) for gage array
%                     part2_excv  = excessVec0(p2_split:p3_split-1,:); % part 2 (gage) for gage array
%                     part1_targ  = targetMatrix0(1:p2_split-1,:); % part 1 (applied loads) for load array
%                     part2_targ  = targetMatrix0(p2_split:p3_split-1,:); % part 2 (gage) for gage array
%                     pointID     = targetMatrix0(1:p2_split-1,:); % part 1 (applied loads) for load array
%                     part2_targ  = targetMatrix0(p2_split:p3_split-1,:); % part 2 (gage) for gage array
...
%                     first_p2    = first_col(p2_split:p3_split-1,:); % first column--all rows in part 2
...
function autoapp_Callback(hObject, eventdata, handles)
